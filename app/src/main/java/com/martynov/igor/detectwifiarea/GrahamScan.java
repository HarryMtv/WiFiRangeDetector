...
public final class GrahamScan {
     ...
     */
    protected static boolean areAllCollinear(List<WiFiPoint> points) {

        if(points.size() < 2) {
            return true;
        }

        final WiFiPoint a = points.get(0);
        final WiFiPoint b = points.get(1);

        for(int i = 2; i < points.size(); i++) {

            WiFiPoint c = points.get(i);
     ...
     */
    public static List<WiFiPoint> getConvexHull(int[] xs, int[] ys) throws IllegalArgumentException {

        if(xs.length != ys.length) {
            throw new IllegalArgumentException("xs and ys don't have the same size");
        }

        List<WiFiPoint> points = new ArrayList<>();

        for(int i = 0; i < xs.length; i++) {
            points.add(new WiFiPoint(xs[i], ys[i]));
        }
     ...
     */
    public static List<WiFiPoint> getConvexHull(List<WiFiPoint> points) throws IllegalArgumentException {

        List<WiFiPoint> sorted = new ArrayList<>(getSortedPointSet(points));
        ...
        Stack<WiFiPoint> stack = new Stack<>();
        stack.push(sorted.get(0));
        stack.push(sorted.get(1));

        for (int i = 2; i < sorted.size(); i++) {

            WiFiPoint head = sorted.get(i);
            WiFiPoint middle = stack.pop();
            WiFiPoint tail = stack.peek();
     ...
     *               is returned.
     */
    protected static WiFiPoint getLowestPoint(List<WiFiPoint> points) {

        WiFiPoint lowest = points.get(0);

        for(int i = 1; i < points.size(); i++) {

            WiFiPoint temp = points.get(i);
     ...
     */
    protected static Set<WiFiPoint> getSortedPointSet(List<WiFiPoint> points) {

        final WiFiPoint lowest = getLowestPoint(points);

        TreeSet<WiFiPoint> set = new TreeSet<>((a, b) -> {
     ...
     *         <code>c</code>.
     */
    protected static Turn getTurn(WiFiPoint a, WiFiPoint b, WiFiPoint c) {
